        ...
        <div className="bg-card text-text border border-border-var rounded-2xl p-6 sm:p-8 shadow-2xl">
          ...
          {showAdvanced && (
            <div className="mt-3 mb-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-sm text-muted">重启次数 (restarts)</label>
                <input
                  type="number"
                  min={1}
                  step={1}
          ...
          {lpplResult && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className={`${getRiskBg(lpplResult.riskLevel)} rounded-xl p-6`}>
              ...
              <div className="bg-panel border border-border-var rounded-xl p-6">
                <h3 className="text-lg font-semibold text-text mb-2">预测临界点</h3>
                <p className="text-2xl font-bold text-info">
                  {lpplResult.criticalDate?.toLocaleDateString("zh-CN")}
                </p>
                <p className="text-sm text-muted mt-1">
                  {Math.round(
                    (lpplResult.criticalDate!.getTime() - new Date(customEnd).getTime()) /
                      (1000 * 86400)
                  )}{" "}
                  天后
                </p>
              ...
              <div className="bg-panel border border-border-var rounded-xl p-6">
                <h3 className="text-lg font-semibold text-text mb-2">模型拟合度</h3>
                <p className="text-2xl font-bold text-accent">{lpplResult.residual.toFixed(2)}</p>
          ...
          <div className="bg-card text-text p-6 shadow-lg rounded-xl">
            <h2 className="text-2xl font-bold mb-4 text-text">价格与 LPPL 拟合曲线</h2>
            <PriceChart data={chartData} priceFormatter={priceFormatter} />
          </div>

          {lpplResult?.params && (
            <div className="mt-8 bg-panel p-6 rounded-xl">
              <h2 className="text-2xl font-bold mb-4 text-text">LPPL 模型参数</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <p className="text-muted text-sm">基线 A (log)</p>
                  <p className="text-text font-mono">{lpplResult.params.A.toFixed(3)}</p>
                </div>
                <div>
                  <p className="text-muted text-sm">基线 A (price)</p>
                  <p className="text-text font-mono">{priceFormatter(Math.exp(lpplResult.params.A))}</p>
                </div>
                <div>
                  <p className="text-muted text-sm">临界时间 (tc)</p>
                  <p className="text-text font-mono">{lpplResult.params.tc.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-muted text-sm">幂律指数 (m)</p>
                  <p className="text-text font-mono">{lpplResult.params.m.toFixed(3)}</p>
                </div>
                <div>
                  <p className="text-muted text-sm">角频率 (ω)</p>
                  <p className="text-text font-mono">{lpplResult.params.omega.toFixed(3)}</p>
                </div>
                <div>
                  <p className="text-muted text-sm">相位 (φ)</p>
                  <p className="text-text font-mono">{lpplResult.params.phi.toFixed(3)}</p>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div className="flex gap-4">
                  <div>
                    ...
                    <p className="text-text font-mono">{(lpplResult.sse ?? 0).toFixed(4)}</p>
                  </div>
                  <div>
                    <p className="text-muted text-xs">RMSE (log-space)</p>
                    <p className="text-text font-mono">{(lpplResult.rmse ?? 0).toFixed(6)}</p>
                  </div>
                  <div>
                    <p className="text-muted text-xs">迭代次数</p>
                    <p className="text-text font-mono">{lpplResult.iterations ?? "-"}</p>
                  </div>
                  <div>
                    <p className="text-muted text-xs">耗时</p>
                    <p className="text-text font-mono">{lpplResult.runTimeMs ?? 0} ms</p>
                  </div>
                </div>

                <div>
                  <p className="text-muted text-xs">拟合状态</p>
                  <p className={`font-mono ${lpplResult.converged ? "text-success" : "text-warning"}`}>
